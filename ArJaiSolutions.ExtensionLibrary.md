## [ArJaiSolutions.ExtensionLibrary](ArJaiSolutions.ExtensionLibrary)
...
| `Nullable<DateTime>` | ToDate(this `String` s, `String` format = ddMMyyyy, `String` cultureString = en-GB) | Converts the given string to Datetime based on the format and culture provided  or by Default the format is "ddMMyyyy" and culture is "en-GB" |
| `Nullable<DateTime>` | ToDateTime(this `String` s, `DateTime` defaultValue = null) | Converts the given string to Datetime.  if Convertion fails and there is a Default value provided then return defaultValue  else tries for another converion mechanism and if that too fails then trows an exception |
| `Nullable<DateTime>` | ToDateTime(this `String` s, `CultureInfo` culture, `String` format = dd/MM/yyyy HH:mm:ss) | Converts the given string to Datetime.  if Convertion fails and there is a Default value provided then return defaultValue  else tries for another converion mechanism and if that too fails then trows an exception |
| `Nullable<DateTime>` | ToDateTime(this `String` s, `String` format, `CultureInfo` culture) | Converts the given string to Datetime.  if Convertion fails and there is a Default value provided then return defaultValue  else tries for another converion mechanism and if that too fails then trows an exception |
